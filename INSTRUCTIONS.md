...
```jsonc
{
  ...
  "skills": {
    ...
    "dirs": []                             // 技能目录；空 = 默认 <cwd>/skills 与 ~/.config/sangxia/skills；
                                           // 非空则**替换**默认列表（相对路径按 session cwd 解析）
  },
...
- **命名**：远端工具以 `mcp__<server>__<tool>` 暴露给模型，避免与内置工具/彼此重名（server 名里非 `[a-zA-Z0-9_-]` 的字符会先替换成 `_`；hook 的 `matcher` 按这个注册名匹配）。
...
- **发现目录**：默认 `<cwd>/skills` 与 `~/.config/sangxia/skills`（同名时项目优先，即项目技能盖过全局同名技能）；也可以用 `config.skills.dirs` 显式指定目录列表（相对路径按会话 cwd 解析）——注意 `dirs` **非空即替换**默认列表，那时全局技能目录不会自动参与，需要自己把它一并列进去。
...
- 会话历史默认持久化到 `~/.config/sangxia/sessions/<sessionId>.jsonl`（append-only 事件流，见「会话持久化」），可用 `SANGXIA_SESSION_DIR` 修改目录；支持 ACP `session/load` 恢复历史。
...
- **斜杠命令**（纯客户端本地命令）：
  - `/model [modelId]` 查看/切换模型（配置 `provider.models`；无参数弹选择器）。空闲时立即生效；turn 进行中切换则**下一轮生效**，状态栏带 `*` 待生效标记。
...
npm run smoke:setup  # setup 向导（headless 路径/验证回退/保留旧配置/权限）
npm run smoke:auth   # 未配置或凭据无效时的 ACP 握手、认证拒绝、环境变量启动与 CLI
```
...
- Hook v2：**每个会话结束时的** `session_end`（目前只有 agent 进程退出时统一跑一次，因为 ACP 0.4.5 没有会话结束事件）、`pre_compact` / `subagent_*`、`turn_end` 的 `decision: "continue"`（要求模型再跑一轮）、结构化注入（图片/文件引用）、可选审计文件（`hooks.auditFile`）。
