 ...
 * - `SANGXIA_LOG_DIR=<dir>`: per-session mode. Each session writes to
 ...
 *   prompt/cancel handlers in. When both SANGXIA_LOG_DIR and SANGXIA_LOG_FILE are
 *   set, SANGXIA_LOG_DIR wins.
 * - `SANGXIA_LOG_FILE=<path>` (legacy): every line goes to one fixed file.
 *
 * Other env vars: SANGXIA_LOG_LEVEL (error|warn|info|debug, default info),
 * SANGXIA_LOG_TIMEZONE (IANA name, default host local timezone).
 *
...
let logFile = process.env.SANGXIA_LOG_FILE;
let logDir = process.env.SANGXIA_LOG_DIR;
if (logDir) {
  try {
...
let threshold = LEVELS[(process.env.SANGXIA_LOG_LEVEL ?? "info").toLowerCase() as Level] ?? LEVELS.info;
...
const logTimeZone = process.env.SANGXIA_LOG_TIMEZONE ?? Intl.DateTimeFormat().resolvedOptions().timeZone;
...
export const logger = {
  ...
  /**
   * 把 fn（及其所有异步后代）的日志绑定到指定 session，写入 SANGXIA_LOG_DIR 下的
